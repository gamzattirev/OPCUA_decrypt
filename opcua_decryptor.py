...
def make_keys(client_nonce, server_nonce):
    ...
    print('server_decrypt_aes: ' + str(server_decrypt_aes.cipher.algorithm.key))
    print('client_decrypt_aes: ' + str(client_decrypt_aes.cipher.algorithm.key))
        ...
        try:
            if raw_list[raw_list_n][:4] == b'MSGF':
                ...
                if dst_ip_list[raw_list_n] == client_ip:
                    ...
                    buf = 4
                    for n in range(len(decrypted) - buf):
                        #unpack_data = struct.unpack("<{:d}I", decrypted[n:n+buf])[0]
                        #unpack_data=int.from_bytes(decrypted[n:n+buf].decode(), 'little')

                        #unpack_data = struct.unpack(b'<1I', decrypted[n:n + buf])[0]

                        #unpack_data = unpack_uatype(ua.VariantType.DataValue,Buffer(decrypted[n:n + buf]))
                        unpack_data=struct.unpack('<I', decrypted[n:n + buf])
                        print(unpack_data)
                        #response =  ua_binary.struct_from_binary(type(ReadResponse()), Buffer(decrypted[n:n + buf]))
                        # return unpack_data
